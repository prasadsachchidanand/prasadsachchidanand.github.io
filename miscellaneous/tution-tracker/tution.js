...
const firebaseConfig = {
  ...
  function loadData() {
    db.collection('students').doc('studentsData').get()
      .then((doc) => {
        if (doc.exists) {
          students = doc.data().students;
          nextStudentId = doc.data().nextStudentId || 1;
          updateStudentDropdown();
          renderStudents();
        } else {
          console.log('No students data found in Firestore');
        }
      })
      .catch((error) => console.error('Error loading students: ', error));
  
    db.collection('sessions').doc('sessionsData').get()
      .then((doc) => {
        if (doc.exists) {
          sessions = doc.data().sessions;
          nextSessionId = doc.data().nextSessionId || 1;
          renderSessions();
          renderSummary();
        } else {
          console.log('No sessions data found in Firestore');
        }
      })
  ...
  function saveData() {
    db.collection('students').doc('studentsData').set({ students, nextStudentId })
      .then(() => console.log('Students saved to Firestore'))
      .catch((error) => console.error('Error saving students: ', error));
  
    db.collection('sessions').doc('sessionsData').set({ sessions, nextSessionId })
      .then(() => console.log('Sessions saved to Firestore'))
